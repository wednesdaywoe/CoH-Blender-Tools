...
| Assemble objects onto the skeleton | Working — CoH character/costume geos author each piece in bone-local space (centred at origin), so on import each object is moved to its root bone on the target armature (from the model name, e.g. `GEO_N_LlegL_…` → `LlegL`, with a weighted-bone fallback). This assembles the figure — the Geopy `import_geo` behaviour. **Attach Objects to Bones** in the import options (default on); object transform only, so export is unaffected |
| Skip LOD meshes on import | Working — a CoH `.geo` ships 2-3 reduced distance-LOD copies of every part (`*_LOD1`, `*_LOD2`) stacked on the full mesh. **Skip LOD Meshes** in the import options (default on) imports only the full-detail piece for a clean, editable scene; untick it to bring in every model (e.g. to re-export a complete `.geo`) |
| Auto bind-pose skeleton on import | Working — a skinned `.geo` with no armature present builds a rest-pose armature at the correct joint positions (male/fem/huge, auto-detected) and binds to it, so the mesh deforms around real joints with no manual skeleton step. Bone positions are validated bit-exact (to 1e-4) against the game's own animation data — every bone, fingers included |
| Skeleton import from `.anim` | Working — **File → Import → CoH Skeleton (.anim)** builds a correctly-positioned rest armature from any full-body `.anim` (reconstructing joints from each bone's frame-0 offset, down an embedded hierarchy if the file has one, else the standard humanoid one). This is the Geopy `import_skel` equivalent. Choose +Y-nub vs toward-child tails and feet-on-ground vs hip-centred anchoring |
...
- **Skinned deformation uses a canonical bind-pose skeleton**: importing a skinned `.geo` with no armature present auto-builds a rest skeleton at the correct joint positions (reconstructed from the game's own animations, for male/fem/huge — pick the body type in the import options or let it auto-detect from the filename). The joint positions are validated bit-exact (to 1e-4) against the game's own animation data for all three body types — every bone including fingers. They also match the leaked dev skeletons (`cohbodies.blend`) on the whole deform body; only the dev rigs' finger/face sub-bones differ slightly, and there our tables track the actual game data. Bones use the game's canonical names (`Hips`, `UarmR`, …), so imported meshes also bind by name directly to a real CoH skeleton if you bring your own. Bone tails default to pointing at their child joint (easier to hand-pose); switch **Bone Tails → +Y Nub** in the import options to match Geopy/the dev rigs exactly for 1:1 game `.anim` playback. The auto rig is hip-centred so it overlays the (hip-centred) imported mesh; to line up with a Geopy/`cohbodies` rig instead, build it feet-on-ground via `create_bind_pose_armature(ground_anchored=True)` or the **Import CoH Skeleton (.anim)** anchor option. Remaining caveat: it's the *standard* humanoid rig, so unusually-proportioned custom bodies won't match exactly. `Create CoH Armature` still exists but stacks every bone at the origin — prefer the auto bind-pose (or **Import CoH Skeleton (.anim)**) for anything you intend to pose.
